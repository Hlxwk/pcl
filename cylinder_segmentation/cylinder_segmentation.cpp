...
main (int argc, char** argv)
{
  ...
  pcl::PCDReader reader;//pcd::io::loadPCDfile
  ...
  pass.setFilterLimits (0, 1.5);//z轴0-1.5
  ...
  std::cerr << "Plane coefficients: " << *coefficients_plane << std::endl;//<<重载
  ...
  extract.filter (*cloud_plane);//提取出平面
  std::cerr << "PointCloud representing the planar component: " << cloud_plane->points.size () << " data points." << std::endl;
  writer.write ("table_scene_mug_stereo_textured_plane.pcd", *cloud_plane, false);

  // Remove the planar inliers, extract the rest
  extract.setNegative (true);
  extract.filter (*cloud_filtered2);//提取平面后剩余的
  extract_normals.setNegative (true);
  extract_normals.setInputCloud (cloud_normals);
  extract_normals.setIndices (inliers_plane);
  extract_normals.filter (*cloud_normals2);//提取后剩余的
